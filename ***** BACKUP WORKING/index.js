...
app.get('/users/:userid', async (req, res) => {
  try {
    ...
    const finalResult = rows.filter((element)=>element.userid===Number(req.params.userid));
...
app.put('/users/:userid', async (req, res) => {
  try {
    ...
    if(finalResult?.length){
      ...
      const result = await connection.promise().query(query, [req.body, req.params.userid]);
...
app.patch('/users/:userid', async (req, res) => {
  try {
    ...
    if(finalResult?.length){
      // Update user in the database
      const query = 'UPDATE user SET ? WHERE userid=?';
      const [rows] = await connection.promise().query(query, [userData, userId]);
      res.send(rows);
...
app.post('/users', async (req, res) => {
  try {
    ...
    const finalResult = getRows.filter((element)=>element.emailid===req.body.emailid);

    if(!finalResult?.length){     
          const query = 'INSERT INTO user (name, emailid, password) VALUES (?, ?, ?)';
          const [result] = await connection.promise().query(query, [userData.name, userData.emailid, userData.password]);
